...
double _mapX(Matrix4 transform, double x, double y) => transform.storage[0] * x +
          transform.storage[4] * y +
          transform.storage[12];

double _mapY(Matrix4 transform, double x, double y) => transform.storage[1] * x +
          transform.storage[5] * y +
...
// http://www.w3.org/TR/SVG/implnote.html#ArcConversionEndpointToCenter
  List<CubicBezier> arcToCubics(
    ...
    if (rx == 0 || ry == 0) {
      return [CubicBezier(x1: currentX, y1: currentY, x2: currentX,y2: currentY,x: arc.x, y: arc.y)];
    }
    ...
    final double midPointDistanceX =
        (currentX - arc.x) * 0.5;
    final double midPointDistanceY =
        (currentY - arc.y) * 0.5;

    final Matrix4 pointTransform = Matrix4.identity();
    pointTransform.rotateZ(-angle);

    final double transformedMidPointX = _mapX(pointTransform, midPointDistanceX, midPointDistanceY);

    final double transformedMidPointY = _mapY(pointTransform, midPointDistanceX, midPointDistanceY);
    ...
    final double centerPointX =
        ((point1x + point2x) * 0.5) - deltaY;
    final double centerPointY =
        ((point1y + point2y) * 0.5) + deltaX;

    final double theta1 = math.atan2(point1y - centerPointY, point1x - centerPointX);
    final double theta2 = math.atan2(point2y - centerPointY, point1x - centerPointX);

    double thetaArc = theta2 - theta1;

    if (thetaArc < 0.0 && arc.sweepFlag) {
      thetaArc += _twoPiFloat;
    } else if (thetaArc > 0.0 && !arc.sweepFlag) {
      thetaArc -= _twoPiFloat;
    }
    ...
    for (int i = 0; i < segments; ++i) {
      ...
      final CubicBezier cubicOperation = CubicBezier(
        x1: _mapX(pointTransform, point1x, point1y),
        y1: _mapY(pointTransform, point1x, point1y),
        x2: _mapX(pointTransform, point2x, point2y),
        y2: _mapY(pointTransform, point2x, point2y),
        x: _mapX(pointTransform, targetPointX, targetPointY),
        y: _mapY(pointTransform, targetPointX, targetPointY),
      );
